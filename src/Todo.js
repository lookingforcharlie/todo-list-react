        ...
        {editable ? 
            <div>
                <form onSubmit={handleSubmit}>
                    ...
                    <input type="text" defaultValue={todo.name} ref={todoEditRef} autoFocus/>
                    <button type="submit">Save</button>
                    <button onClick={handleEditable} type="button">Back</button>
                </form>
            </div>
            :
            <label className='todoItem'>
                <input type="checkbox" checked={todo.isComplete} onChange={handleToggle} />
                <span className={todo.isComplete ? "completed" : "notCompleted"}>{todo.name}</span>
